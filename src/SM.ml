...
| insn :: prg' ->
   eval env
     (match insn with
      ...
      | CALL name      -> name; ((prg', st)::cstack, stack, c)
     )
...
let compile (defs, p) = 
  ...
  [JMP "main"] @ compiled_defs @ [LABEL "main"] @ compiled_s
