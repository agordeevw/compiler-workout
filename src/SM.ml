...
| insn :: prg' ->
  eval env (
    ...
    | BEGIN (_, args, ls) -> 
      ...
      let st', stack' = List.fold_left (fun (s, (z :: stck)) x -> State.update x z s, stck) (inner_st, stack) (List.rev args) in
      (cstack, stack', (st', i, o))
    | END | RET _    ->
      (match cstack with 
      | [] -> conf
...
let compile (defs, p) = 
  ...
  and expr = (function
    ...
    | Expr.Call (f, es)     -> call (label f) es false
  ) 
  ...
  let rec compile_stmt l env = (function
    ...
    | Stmt.Call (name, args) ->
        env, false, call (label name) args true
    | Stmt.Return optexpr ->
        env, false, 
